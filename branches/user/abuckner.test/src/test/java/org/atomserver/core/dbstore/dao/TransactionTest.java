...
public class TransactionTest extends DAOTestCase {
    ...
    private void runTimestampBehaviorTest(final TransactionTemplate template,
                        ...
                        template.execute(new TransactionCallbackWithoutResult() {
                            public void doInTransactionWithoutResult(TransactionStatus transactionStatus) {
                                ...
                                List<EntryMetaData> list = entriesDAO.getWriteEntriesDAO().selectFeedPage(
                                        new Date(0L), AtomServerConstants.FAR_FUTURE_DATE, startIndex, -1, 10, false, entryIn.getLocale().toString(),
        ...
        List<EntryMetaData> list = entriesDAO.selectFeedPage(
                null, AtomServerConstants.FAR_FUTURE_DATE, startIndex, -1, 1, false, entryIn.getLocale().toString(),
                new BaseFeedDescriptor(entryIn.getWorkspace(), entryIn.getCollection()),
                null);
